...
def recall_loss(norm_rank: In[Array[float]], size: In[int], label: In[Array[int]], loss: Out[float]):
    ...
    while (i < size, max_iter := 10000):
        # denormalize ranks
        norm_rank[i] = norm_rank[i] * size
        loss = loss + norm_rank[i] * label[i]
...
def rank_gradient(score: In[Array[float]], pos_score: In[Array[float]], size: In[int], rank: In[Array[int]], pos_rank: In[Array[int]], norm_rank: In[Array[float]],
                  pos_norm_rank: In[Array[float]], d_norm_rank: Out[Array[float]], label: In[Array[int]], d_label: In[Array[int]]):
    ...
    while (i < size, max_iter := 10000):
        ...
        pos_score[i] = score[i] - 0.02 * deviation
    RaMBO_forward(pos_score, size, rank, norm_rank)
    ...
    while (i < size, max_iter := 10000):
        norm_rank[i] = (norm_rank[i] - pos_norm_rank[i]) * label[i]

    d_size: int
    loss: float

    d_rev_recall_loss(norm_rank, d_norm_rank, size,
                      d_size, label, d_label, loss)
...
def RaMBO_backward(score: In[Array[float]], d_score: Out[Array[float]], score_prime: In[Array[float]],
                   ...
                   d_norm_rank: In[Array[float]], lambda_val: In[float]):
...
def call_RaMBO(score: In[Array[float]], d_score: Out[Array[float]], pos_score: In[Array[float]], size: In[int], rank: In[Array[int]], pos_rank: In[Array[int]],
               norm_rank: In[Array[float]], pos_norm_rank: In[Array[float]], d_norm_rank: In[Array[float]], label: In[Array[int]], d_label: In[Array[int]],
               lambda_val: In[float]):
    rank_gradient(score, pos_score, size, rank, pos_rank,
                  norm_rank, pos_norm_rank, d_norm_rank, label, d_label)

    # restore score and norm_rank
